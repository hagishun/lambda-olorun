...
let identityManagerMgr = new IdentityManagerMgr(
  ethereumMgr,
  privateBlockchainMgr
);
let metaTxMgr = new MetaTxMgr(ethereumMgr, privateBlockchainMgr);

let requestTokenHandler = new RequestTokenHandler(
  uPortMgr,
  privateBlockchainMgr
);
let createIdentityHandler = new CreateIdentityHandler(
  uPortMgr,
  privateBlockchainMgr,
  identityManagerMgr
);
...
module.exports.requestToken = (event, context, callback) => {
  preHandler(requestTokenHandler, event, context, callback);
};
module.exports.createIdentity = (event, context, callback) => {
  preHandler(createIdentityHandler, event, context, callback);
};
