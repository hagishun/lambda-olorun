  ...
  async handle(event, context, cb) {
    ...
    try {
      ...
      const {
        managerAddress,
        txHash
      } = await this.identityManagerMgr.createIdentity(identityOpts);
    ...
    while (idAddress == null) {
      try {
        ...
        idAddress = await this.identityManagerMgr.getIdentityFromTxHash(
          idCreationtxHash,
          networkId
        );
    ...
    console.log("networkId", networkId, "idAddress", idAddress);
    let privProv = {
      aud: profile.address,
      sub: encode({
        network: networkId,
    ...
    try {
      ...
      await this.uPortMgr.push(
        profile.pushToken,
        profile.publicEncKey,
        privProvUrl
      );
      console.log("pushed.");
    } catch (error) {
