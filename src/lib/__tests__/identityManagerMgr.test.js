...
describe("IdentityManagerMgr", () => {
  ...
  beforeAll(() => {
    ...
    mockEthereumMgr.getProvider.mockImplementation(() => {
      return {};
    });
  ...
  describe("initIdentityManager", () => {
    ...
    test("no networkId", done => {
      sut
        .initIdentityManager("IdentityManager", null)
        .then(resp => {
          fail("shouldn't return");
          done();
        })
    ...
    test("invalid managerType", done => {
      sut
        .initIdentityManager("invalidManager", networkName)
        .then(resp => {
          fail("shouldn't return");
          done();
        })
  ...
  test("createIdentity() no managerType", done => {
    sut
      .createIdentity({ deviceKey: deviceKey, blockchain: networkName })
      .then(resp => {
        fail("shouldn't return");
        done();
      })
  ...
  test("storeIdentityCreation() no txHash", done => {
    sut
      .storeIdentityCreation(deviceKey, null, null, null)
      .then(resp => {
        fail("shouldn't return");
        done();
      })
  ...
  test("storeIdentityCreation() no networkName", done => {
    sut
      .storeIdentityCreation(deviceKey, txHash, null, null)
      .then(resp => {
        fail("shouldn't return");
        done();
      })
  ...
  test("storeIdentityCreation() no managerType", done => {
    sut
      .storeIdentityCreation(deviceKey, txHash, networkName, null)
      .then(resp => {
        fail("shouldn't return");
        done();
      })
  ...
  test("storeIdentityCreation() no manager", done => {
    sut
      .storeIdentityCreation(deviceKey, txHash, networkName, managerAddress)
      .then(resp => {
        fail("shouldn't return");
        done();
      })
  ...
  test("getIdentityCreation() no pgUrl", done => {
    ...
    sut
      .getIdentityCreation(deviceKey, networkName)
      .then(resp => {
        fail("shouldn't return");
        done();
      })
  ...
  test("getIdentityCreation() happy path", done => {
    ...
    sut.getIdentityCreation(deviceKey, networkName).then(resp => {
      expect(pgClientMock.connect).toBeCalled();
      expect(pgClientMock.query).toBeCalled();
      expect(pgClientMock.query).toBeCalledWith(
  ...
  test("getIdentityFromTxHash() no blockchain", done => {
    sut
      .getIdentityFromTxHash(txHash, null)
      .then(resp => {
        fail("shouldn't return");
        done();
      })
  ...
  test("getIdentityFromTxHash() no pgUrl", done => {
    ...
    sut
      .getIdentityFromTxHash(txHash, networkName)
      .then(resp => {
        fail("shouldn't return");
        done();
      })
  ...
  test("getIdentityFromTxHash() no txReceipt", done => {
    sut.setSecrets({ PG_URL: "fake" });
    mockEthereumMgr.getTransactionReceipt = jest.fn(() => {
      return Promise.resolve(null);
    });
    mockEthereumMgr.getTransactionReceipt.mockClear();
    sut.getIdentityFromTxHash(txHash, networkName).then(resp => {
  ...
  test("getIdentityFromTxHash() happy path", done => {
    sut.setSecrets({ PG_URL: "fake" });
    sut.decodeLogs = jest.fn(() => {
      return Promise.resolve({ identity: deviceKey });
    });

    mockEthereumMgr.getTransactionReceipt = jest.fn(() => {
      return Promise.resolve(txReceiptMock);
    });
    mockEthereumMgr.getTransactionReceipt.mockClear();
    pgClientMock.connect = jest.fn();
    pgClientMock.connect.mockClear();
